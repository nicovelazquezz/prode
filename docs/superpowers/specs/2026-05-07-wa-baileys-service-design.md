...
`security-use-guards`, `security-validate-all-input`. No global `@nestjs/throttler`: the only caller is the prode backend on the internal Dokploy network with Bearer auth, and the real outbound throttle lives inside `BaileysClientService` as `WA_SEND_DELAY_MS`.
...
Multi-stage, Node 20 alpine. A `.dockerignore` at `wa-backend/` excludes `data/`, `node_modules/`, `dist/`, and `.env*` so dev session credentials never end up in the image.
...
- [ ] `backend/.env.example` and Dokploy env updated to `WHATSAPP_API_URL=http://wa-backend:3001`.
- [ ] Existing notification BullMQ jobs deliver successfully end-to-end.
